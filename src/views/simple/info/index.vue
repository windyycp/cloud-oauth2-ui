<template>
  <div>
    <InfoPage>
      <template v-slot:title>{{ $t('message.router.simple') }}</template>
      <template v-slot:control>
        <Auth :value="'btn.syslog.edit'">
          <el-button type="primary" v-if="state.infoMode" :icon="Edit" link
                     @click="handleEdit"> {{ $t('message.formI18n.but_edit') }}
          </el-button>
          <el-button type="info" v-if="!state.infoMode" :icon="Edit" link
                     @click="handleCancel">{{ $t('message.formI18n.but_cancel') }}
          </el-button>
          <el-button type="success" v-if="!state.infoMode" :icon="Edit" link
                     @click="submitForm(formRef)">{{ $t('message.formI18n.but_save') }}
          </el-button>
        </Auth>
      </template>

      <template v-slot:form>
        <el-form ref="formRef" :model="state.form" size="default" :disabled="false" :hide-required-asterisk="state.infoMode">
...
import {get, update,} from "/@/api/simple/index";
import {useRouter} from "vue-router";
import {Edit} from "@element-plus/icons-vue";
import {ElMessage} from "element-plus";
import {getAllDataConfig, getQueryDataConfig} from "/@/model/simple/dataConfig";
import Auth from "/@/components/auth/auth.vue";
import InfoPage from "/@/layout/page/infoPage.vue";
import {fastFormDataConfig} from "/@/types/fastForm";
import {getConfigWithLabelTag, getEditDataConfig} from "/@/utils/lowcode";
...
var formList = computed(() => {
  // 筛选出详情还是编辑参数列表
  const filterDataConfig = getEditDataConfig(dataConfig, state.infoMode)
  // 按tagLabel分组
  return getConfigWithLabelTag(filterDataConfig)
})
